 ...
 * Description: Displays the Accounts interface with a list of accounts, inline editing,
 *              Ajax deletion, and a form to add new accounts. Now includes client_id_enc,
 *              client_secret_enc, refresh_token_enc fields in the add form.
 */
...
// 1) Get list of projects (for selecting a project when adding an account)
$projects_manager = new SDM_Projects_Manager();
$all_projects = $projects_manager->get_all_projects();

// 2) Get list of existing accounts
...
<div class="wrap">
    ...
    <!-- Notice container for accounts (inline editing, add/delete) -->
                ...
                <?php foreach ( $accounts as $account ) : ?>
                    <tr id="account-row-<?php echo esc_attr( $account->id ); ?>"
                        ...
                        <td class="column-project">
                            ...
                            <!-- If you want inline editing for project_id, show an input or select here -->
                            <input class="sdm-edit-input sdm-hidden" type="number" name="project_id" value="<?php echo esc_attr( $account->project_id ); ?>">
                        </td>
                        ...
                        <td class="column-api-key">
                            <span class="sdm-display-value">
                                <?php 
                                // Instead of showing real encrypted text, show "Encrypted" if not empty
                                echo ( ! empty( $account->api_key_enc ) ) ? esc_html__( 'Encrypted', 'spintax-domain-manager' ) : ''; 
                                ?>
                            </span>
                            <!-- If user wants to set a new key, they can type it; leaving empty means keep old -->
                            <input class="sdm-edit-input sdm-hidden" type="text" name="api_key_enc" value=""
                                placeholder="<?php esc_attr_e('Leave empty to keep existing key', 'spintax-domain-manager'); ?>">
                        </td>
                        ...
                        <td class="column-additional-data">
                            ...
                            <textarea class="sdm-edit-input sdm-hidden" name="additional_data_enc" rows="2"
                                placeholder="<?php esc_attr_e('Leave empty to keep existing data', 'spintax-domain-manager'); ?>"></textarea>
                        </td>
    ...
    <?php else : ?>
        ...
        <form id="sdm-add-account-form">
            ...
            <table class="form-table">
                ...
                <!-- Additional optional fields: client_id_enc, client_secret_enc, refresh_token_enc -->
                <tr>
